...
fn main() {
    ...
    let console =
        AsyncConsole::<State, Bye>::builder_with_reason(runtime.handle().clone())
            .text(Text {
                command_panicked: "异步指令 panic；控制台已兜住".to_owned(),
                ..Text::default()
            })
            .prompt("async> ")
            .command(literal("ping").describe("立即回应").executes_async(
                |ctx: &CommandContext<Src>| {
                    let source = ctx.source.as_ref().clone();
                    async move {
                        source.printer().print("pong");
                        1
                    }
                },
            ))
            .command(literal("slow").describe("异步等待三秒").executes_async(
                |ctx: &CommandContext<Src>| {
                    let source = ctx.source.as_ref().clone();
                    async move {
                        source.printer().print("开始等待；现在可以继续输入 ping");
                        tokio::time::sleep(Duration::from_secs(3)).await;
                        source.printer().print("等待结束");
                        1
                    }
                },
            ))
            ...
            .command(literal("stop").describe("退出").executes_async(
                |ctx: &CommandContext<Src>| {
                    let source = ctx.source.as_ref().clone();
                    async move {
                        source.request_exit(Bye::Stop);
                        1
                    }
                },
            ))
